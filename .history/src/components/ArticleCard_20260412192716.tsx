    ...
    <article className={`${styles.card} ${featured ? styles.featured : ''}`}>
      {article.heroImage && (
        <Link href={href} className={styles.imageWrap}>
          <Image
						src={urlFor(article.heroImage).width(featured ? 1400 : 600).height(featured ? 1050 : 400).url()}
						alt={article.title}
						fill
						sizes={featured ? '(max-width: 768px) 100vw, 50vw' : '(max-width: 640px) 100vw, (max-width: 1024px) 33vw, 25vw'}
      ...
      <div className={styles.body}>
        ...
        <div className={styles.meta}>
          ...
          {date && <span className={styles.dot} style={{fontFamily: 'var(--font-mono)', fontSize: '0.7rem', letterSpacing: '0.06em', color: 'var(--muted)'}}>{date}</span>}
        </div>
