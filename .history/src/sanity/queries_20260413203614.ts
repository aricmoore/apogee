...
export async function getAllEvents(): Promise<Event[]> {
  return client.fetch(`
    *[_type == "event"] | order(date asc) {
      _id, title, slug, date, endDate, location, venue, address,
      category, description, heroImage, externalLink, featured, soldOut
    }
  `)
}

export async function getSiteSettings(): Promise<{ featuredArticle: Article } | null> {
  return client.fetch(`
